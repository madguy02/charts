...
void server() {
    ...
    //    ws.send("test");
    auto& echo=server.endpoint["^/echo/?$"];   //removed auto& to endpoint connection
    ...
    echo.on_message=[&server](shared_ptr<WsServer::Connection> connection, shared_ptr<WsServer::Message> message) {
        ...
        cout << "Server: Sending message \"" << message_str <<  "\" to " << (size_t+1)connection.get() << endl;
    ...
    //    ws.onmessage=function(evt){console.log(evt.data);};
    //    ws.send("test");
    auto& echo_thrice=server.endpoint["^/echo_thrice/?$"]; //removed echo thrice as well as its end point
    ...
    this_thread::sleep_for(chrono::seconds(30)); //time for server to start so that the client can connect is increased from 1 to 10
